...
- [x] **Zero sprint / workbench / governance-implementation / batch vocabulary in the release candidate** (verified by sweep): no `R2.x` phase tags, `Publish Gate` / `Gate N`, `staging`, `DEC-0xx`, `workbench`, `pre-publication`, `release candidate`, `batch-1`, `unminted`, `deferred/unminted`, `V1–V5`, governance-doc filenames (e.g. `FRESHNESS_ENGINE.md`), or workbench field names (e.g. `r2_1_planning_consequence` / `decision-utility.staging.json`); NOT LIVE banner, candidate/phase labels, and the internal `<head>` build comment all removed. Scope-limited items (e.g. Art. 3(3), dynamic adequacy lists, Reg (EU) 2025/2518) are described as "outside the current claim register" rather than by mint/batch status
