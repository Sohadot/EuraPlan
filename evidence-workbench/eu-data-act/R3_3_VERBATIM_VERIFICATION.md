...
**Block-boundary seam recorded (continuation of the §17/§25/§34/§43 pattern) — numeric range is authoritative:** numeric range `…100`–`…113` = rows **F6 + G1/G9/G2a/G2b/G2c/G3/G7/G6/G4a/G4b/G5a/G5b/G5c**. The draft-sequence label for Block 7 reads "G1–G8", off on **both** ends: **F6** was pulled into this block (Ch V scope-limit, per the R3.3-F §51 recommendation), and **G8 = R-G8 = Art. 31 (bespoke / non-production exemptions) = `EP-CLM-000114`** falls in Block 8's numeric range — **not** verified here; it begins R3.3-H. **G8 is the Q9 + Q10 exemption carve-out** on G1(23(d))/G4a/G4b(29)/G5a(30(1))/G5c(30(3)), so it is a forward dependency exactly as F6 was for Ch V, E6 for Ch IV, and D10 for Ch III (see G1/G4a/G5a below). **Per instruction, the numeric range governs; no `EP-CLM` is renumbered to absorb the label mismatch — the mismatch is recorded here.** (Note: `EP-CLM-000102` carries the draft label "G9" (Art. 24 scope-limiter) sitting between G1 and G2a in numeric order — an intra-series ordering artefact, not a seam; it is verified here in numeric position.)
...
- **G8 forward dependency (Art 31, `…114`):** the Q9 + Q10 exemption (bespoke custom-built + non-production test/eval) is **not** verified here — it falls in Block 8's numeric range and begins R3.3-H. G1/G4a/G4b/G5a/G5c must render with G8 present once publication stages begin. **Flagged forward (mirrors F6/E6/D10).**
