...
E = {
 ...
 57:dict(claim="The Chapter II obligations do not apply to data generated through the use of connected products manufactured or designed, or related services provided, by a microenterprise or small enterprise, provided that enterprise has no partner or linked enterprise (within Article 3 of the Annex to Recommendation 2003/361/EC) that is not itself a micro or small enterprise, and is not subcontracted to manufacture or design the product or provide the service (Article 7(1)).", qb=None, eers=["DIM-01"]),
 58:dict(claim="The same Article 7(1) exemption applies to an enterprise that has qualified as a medium-sized enterprise for less than one year, and to connected products for one year after they are placed on the market by such an enterprise, carrying the same Article 7(1) conditions rather than as an unconditional exemption (Article 7(1), second subparagraph).", qb=None, eers=["DIM-01"]),
 ...
 93:dict(claim="The Article 15(1)(b) non-emergency route does not apply to microenterprises and small enterprises (Article 15(2)).", qb=None, eers=["DIM-01"]),
...
# claim_risk per CLAIM_POLICY v1.1: Low only for bibliographic instrument identity (A1).
# Entry-into-force (A2/000047) is a legal instrument-effect claim under v1.1 -> High.
LOW = {46}
...
# locked corrections (must not regress)
dim04 = {c["id"] for c in claims if "DIM-04" in c["affected_eers_dimensions"]}
assert dim04 == {C(123), C(124)}, f"DIM-04 exact set must be {{000123,000124}}, got {sorted(dim04)}"
lowset = {c["id"] for c in claims if c["claim_risk"] == "Low"}
assert lowset == {C(46)}, f"Low-risk exact set must be {{000046}}, got {sorted(lowset)}"
