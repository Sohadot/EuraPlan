...
### DEC-057
- **Date:** 2026-08-23 (GDPR development frozen post-R2.8; Sprint R3 — EU Data Act upgrade opened)
- **Status:** Active
- **Decision:** (1) **Freeze `/regulation/gdpr/` (EP-REG-002) development.** With R2.8 closed (DEC-056; live 2026-08-23; RGS v2 97.5), the GDPR Evidence Graph reference enters a **frozen / reference-grade / governed** development state. No cosmetic edits, stylistic re-wording, or non-substantive "improvement" passes may be made. A change to EP-REG-002 (HTML, `claims.json`, Decision Utility, provenance, or metadata) is admissible **only** when it originates from one of four triggers: (a) a **freshness trigger** under FRESHNESS_ENGINE.md (`validity_state: review_required` → resolution driven by a real triggering event; no simulated freshness); (b) a **genuine factual correction** (`corrected`) backed by a re-verification event; (c) a **supersession** (`superseded`) with a superseding source or claim; or (d) a **clear functional need** (accessibility fix, broken-link/citation-integrity repair, schema/machine-representation defect). Every such change still requires its own record, preserves the global opaque ID sequence, and never advances `last_verified_at` or provenance SHAs without the corresponding real event. The publication lifecycle (`published`/`active`) stands until a trigger moves it. **97.5 remains the recorded RGS v2 closeout score** — a result of that audit under the *versioned* scoring standard, not a permanent ontology property of the route; a future revision of the governing scoring standard may trigger a re-score without reopening GDPR development or mutating the route. (2) **Reaffirm the Wave 1 Depth Equalization posture (DEC-047): no expansion** — no new country, sector, funding, brief, matrix, or regulation route beyond the Wave 1 set — **until every Wave 1 Core Authority route scores ≥ 90.** Current Wave 1: EU AI Act 97 (≥90), GDPR 97.5 (≥90), **EU Data Act 58, CRA 60, EERS 62, Protocol 60** (all < 90). Two of six above threshold; four remain. The program has moved from proving the method (AI Act, GDPR) to **repeating the quality across the corpus**; that repetition, not expansion, is the active mandate. (3) **Open Sprint R3 — EU Data Act (EP-REG-003) Evidence Graph-grade upgrade**, next in RGS v2 order (Data Act → CRA → EERS → Protocol). **Method — governance reuse, content rediscovery:** R3 **reuses** the R2 operating system as an operational precedent — REFERENCE_GRADE_ROUTE_STANDARD.md v2 six layers, the frozen Publish-Gate architecture (Gate 0 index control → Gate 6 live re-score), EVIDENCE_GRAPH_MODEL.md, CLAIM_IDENTITY_AND_LIFECYCLE_SPECIFICATION.md, SOURCE_POLICY.md, CLAIM_POLICY.md, and the phase sequence (source discovery → claim map/falsification → human verification → canonical `claims.json` → page transformation → Decision Utility → machine/citation prep → Publish Gate → RGS re-score) — but **re-discovers the Data Act claim set and analytical layer from primary sources from scratch**, according to the material truth of Regulation (EU) 2023/2854 (EUR-Lex CELEX `32023R2854`; European Commission data-economy materials; Tier 1 only). GDPR is precedent, **not** a content template: no GDPR claim, Decision Object, source node, or claim count is ported. (4) **R3 constraints (mirroring DEC-048):** next free claim ID is **`EP-CLM-000046`** — global opaque sequence only, never `DATA-CLM-*` / `DA-CLM-*`; **no `EP-CLM-*` mint before literal falsification**; claim count follows Data Act material truth, not a template; **no** public `regulation/eu-data-act/claims.json`, **no** live HTML rewrite of `/regulation/eu-data-act/`, **no** `routes.json` / `llms.txt` / sitemap alternate registration, and **no** Publish Gate until R3's terminal gate authorizes them under a separate DEC; **no** parallel CRA / EERS / Protocol / country / sector / expansion work while R3 is open. R3.0 scaffolding lives in `evidence-workbench/eu-data-act/`.
- **Rationale:** DEC-056 made GDPR a reference-grade governed asset with a complete proof chain; continuing to polish it would manufacture churn and risk the "cosmetic freshness" FRESHNESS_ENGINE.md forbids, so its development state must be explicitly closed to all but real triggers. With AI Act and GDPR both ≥ 90, the binding question is no longer whether the method works but whether it repeats — so the correct next move is Wave 1 equalization, not expansion, and the Data Act is next in RGS v2 order. Reusing the governance and gate machinery while rediscovering the claim set from primary sources turns GDPR into an operational precedent rather than a content mould, which is what keeps each regulation's claim graph faithful to its own text.
- **Affected routes/files:** `DECISION_LOG.md`; `evidence-workbench/eu-data-act/README.md` (new — R3.0 sprint scaffold); `evidence-workbench/gdpr/README.md` (frozen-state note). No live route, `claims.json`, `routes.json`, `llms.txt`, sitemap, or GDPR/Data Act HTML mutation in this decision.
- **Governance documents involved:** REFERENCE_GRADE_ROUTE_STANDARD.md v2; EVIDENCE_GRAPH_MODEL.md; CLAIM_IDENTITY_AND_LIFECYCLE_SPECIFICATION.md; SOURCE_POLICY.md; CLAIM_POLICY.md; FRESHNESS_ENGINE.md; ROUTE_GOVERNANCE.md; DISCLOSURE_BOUNDARY.md; DEC-047; DEC-048 (R2 opening precedent); DEC-056
- **Reversal conditions:** Superseding DEC required to (a) reopen GDPR cosmetic development, (b) lower the Wave 1 ≥ 90 threshold or open expansion before all Wave 1 routes reach it, (c) cancel or re-order Sprint R3, or (d) port GDPR claim content into the Data Act graph instead of rediscovering it from primary sources.
- **Notes:**
  - **Permitted GDPR changes:** freshness trigger · genuine correction · supersession · clear functional need — each recorded, none cosmetic
  - **Wave 1 order:** Data Act → CRA → EERS → Protocol (DEC-047 / RGS v2)
...
### DEC-059
- **Date:** 2026-08-29 (Sprint 5D framing rejected; serial Wave 1 upgrade path affirmed)
- **Status:** Active
- **Decision:** **Do not admit "Sprint 5D — Wave 1 Quality Equalization" as originally framed** (parallel, single-sprint HTML "quality hardening" of `/regulation/eu-data-act/`, `/regulation/cyber-resilience-act/`, `/standard/eers/`, and `/protocol/` to lift each to RGS v2 ≥ 90). That framing is **superseded before execution** because it conflicts with binding governance and with the RGS v2 scoring model on three points: (1) **DEC-047 / DEC-057** require Wave 1 routes to be raised **serially, in RGS v2 order** (Data Act → CRA → EERS → Protocol), and DEC-057 explicitly forbids **parallel CRA / EERS / Protocol work while Sprint R3 (EU Data Act) is open** — and R3 **is** open; (2) under `REFERENCE_GRADE_ROUTE_STANDARD.md` v2 a route reaches ≥ 90 only when all **six layers** are genuinely present — evidence depth, unique analytical layer, real citation anchors, and a real freshness contract — which is produced by the **Evidence-Graph Publish-Gate process** (as AI Act = 97 and GDPR = 97.5 were), **not** by editing live HTML; (3) `FRESHNESS_ENGINE.md` and DEC-057 forbid **cosmetic freshness** and **simulated citation readiness**, so a score improvement manufactured by HTML edits would be a governance violation and could not be certified. **No CRA / EERS / Protocol parallel hardening was performed. No fabricated ≥ 90 score was claimed. `DEC-047` and `DEC-057` are preserved and remain binding.** The Wave 1 **objective** (all four routes to ≥ 90) is **not abandoned — it is pursued through the already-active serial R-series**: complete **Sprint R3 (EU Data Act, EP-REG-003)** first, then CRA, then EERS, then Protocol, each via the frozen Publish-Gate architecture and its terminal RGS re-score.
- **Rationale:** Sprint 5D was proposed off the DEC-058 (Sprint 5C) recommendation, which itself pointed at the DEC-047 / DEC-057 no-expansion gate. On inspection, the *method* 5D proposed (parallel one-sprint HTML hardening) is the opposite of what that gate requires (serial, evidence-gated upgrades) and would have forced either a governance breach or fabricated scores. Recording the rejection — rather than silently executing or silently dropping 5D — keeps the decision register honest and the audit trail unbroken, and re-points the same objective at the correct, already-running workstream.
- **Affected routes/files:** `DECISION_LOG.md` (this entry). **No** live route, HTML, `routes.json`, `sitemap.xml`, `robots.txt`, `llms.txt`, `claims.json`, or workbench file was mutated by this decision. Data Act R3 continues on its own workbench (`evidence-workbench/eu-data-act/`) and branches.
- **Governance documents involved:** DEC-047; DEC-057; DEC-058; REFERENCE_GRADE_ROUTE_STANDARD.md v2; EVIDENCE_GRAPH_MODEL.md; CLAIM_IDENTITY_AND_LIFECYCLE_SPECIFICATION.md; FRESHNESS_ENGINE.md; SOURCE_POLICY.md; CLAIM_POLICY.md; ROUTE_GOVERNANCE.md
- **Reversal conditions:** A superseding DEC would be required to change the Wave 1 upgrade strategy from serial (Evidence-Graph, RGS-gated) to any parallel or HTML-only method, or to lower/redefine the RGS v2 ≥ 90 threshold. Absent such a DEC, the serial R-series path stands and expansion (Sprint 6A / 6B) stays gated until every Wave 1 route legitimately reaches ≥ 90.
- **Notes:**
  - **5D status:** *as originally framed* = **SUPERSEDED / not admitted**; the underlying objective = **deferred to the serial R-series** (Data Act R3 active; CRA / EERS / Protocol later, in order)
...
*Governed by Sohadot | Established Sprint 4D — June 2026 | Updated Sprint 5C — August 2026 (DEC-059 governance correction: Sprint 5D framing superseded; serial R-series path affirmed)*
